...
class OpenSSLConan(ConfigurableConanFile):
    (
        name,
        version,
        dependencies,
    ...
    source_tgz_old = (
        "https://www.openssl.org/source/old/1.1.0/openssl-%s.tar.gz" % version
    )

    def build_requirements(self):
        # useful for example for conditional build_requires
        if self.compiler == "Visual Studio":
            self.build_requires(self.dependencies["strawberryperl"])
            if not self.options.no_asm:
                self.build_requires(self.dependencies["nasm"])
    ...
    def unix_build(self):
        ...
        self.run_in_src(
            "./Configure %s %s" % (target, self._get_flags()), win_bash=win_bash
        )
    ...
    def _patch_install_name(self):
        ...
        tools.replace_in_file(
            "%s/Makefile" % self.subfolder, old_str, new_str, strict=self.in_local_cache
        )
            ...
            for e in runtimes:
                if e != self.settings.compiler.runtime:
                    try:
                        ...
                        self.output.warn(
                            "replace vs runtime %s in %s" % ("/%s" % e, filename)
                        )
                        replaced = True
                    except:
                        pass
    ...
    def visual_build(self):
        self.output.warn(
            "----------CONFIGURING OPENSSL FOR WINDOWS. %s-------------" % self.version
        )
        target = self._get_target()
        ...
        with tools.vcvars(self.settings, filter_known_paths=False):
            ...
            config_command = "perl Configure %s %s --prefix=%s/binaries %s" % (
                target,
                no_asm,
                self.source_folder,
                self._get_flags(),
    ...
    def package(self):
        ...
        if self.settings.os == "Windows" and self.compiler == "Visual Studio":
            ...
            if self.options.shared:
                self.copy(
                    src=self.subfolder,
                    pattern="*libcrypto.dll.a",
                    dst="lib",
                    keep_path=False,
                )
                self.copy(
                    src=self.subfolder,
                    pattern="*libssl.dll.a",
                    dst="lib",
                    keep_path=False,
                )
                self.copy(
                ...
                self.copy(
                    src=self.subfolder, pattern="*libssl.a", dst="lib", keep_path=False
                )
        else:
            if self.options.shared:
        ...
        self.copy(
            src=self.subfolder, pattern="apps/openssl", dst="bin", keep_path=False
        )
