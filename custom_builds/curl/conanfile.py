...
from conans import AutoToolsBuildEnvironment, RunEnvironment, CMake, tools
...
class GrpcConan(ConfigurableConanFile):
    ...
    def requirements(self):
        ...
        if self.options.with_libssh2:
            if self.settings.compiler != "Visual Studio":
                self.requires.add("libssh2/1.8.0@forwardmeasure/stable")

        self.requires.add(self.dependencies["zlib"])
    ...
    def source(self):
        archive_url = "https://github.com/curl/curl/archive/{}.zip".format(
            self.version
        )
        tools.get(archive_url)
        os.rename("curl-{!s}".format(self.version), self._source_subfolder)
