...
class CppRestSDKConan(ConfigurableConanFile):
    (
        name,
        version,
        dependencies,
    ...
    websocketpp_reference = dependencies["websocketpp"]
    ...
    def source(self):
        ...
        if self.settings.compiler == "clang" and str(self.settings.compiler.libcxx) in [
            "libstdc++",
            "libstdc++11",
        ]:
            tools.replace_in_file(
                path.join(self._source_subfolder, "Release", "CMakeLists.txt"),
                "libc++",
                "libstdc++",
            )
        if self.settings.os == "Android":
            ...
            tools.replace_in_file(
                path.join(self._source_subfolder, "Release", "src", "pch", "stdafx.h"),
    ...
    def _configure_cmake(self):
        if self.settings.os == "iOS":
            with open("toolchain.cmake", "w") as toolchain_cmake:
                ...
                toolchain_cmake.write(
                    'set(CMAKE_XCODE_EFFECTIVE_PLATFORMS "-%s" CACHE STRING "" FORCE)\n'
                    % sdk
                )
                toolchain_cmake.write(
                    'set(CMAKE_OSX_ARCHITECTURES "%s" CACHE STRING "" FORCE)\n' % arch
                )
                toolchain_cmake.write(
                    'set(CMAKE_OSX_SYSROOT "%s" CACHE STRING "" FORCE)\n' % sysroot
                )
            environ["CONAN_CMAKE_TOOLCHAIN_FILE"] = path.join(
                getcwd(), "toolchain.cmake"
            )
        ...
        ] = self.options.exclude_websockets
        cmake.definitions[
            "CPPREST_EXCLUDE_COMPRESSION"
        ] = self.options.exclude_compression
        cmake.definitions["CPPREST_VERSION"] = self.version
        cmake.definitions["OPENSSL_ROOT_DIR"] = self.deps_cpp_info["openssl"].rootpath
        cmake.definitions["OPENSSL_USE_STATIC_LIBS"] = not self.options[
            "openssl"
        ].shared
        if self.settings.compiler == "Visual Studio":
    ...
    def package(self):
        ...
        self.copy(
            pattern="*",
            dst="include",
            src=path.join(self._source_subfolder, "Release", "include"),
        )
    ...
    def package_info(self):
        if self.settings.compiler == "Visual Studio":
            ...
            toolset = {"12": "120", "14": "140", "15": "141"}.get(
                str(self.settings.compiler.version)
            )
            version_tokens = self.version.split(".")
            versioned_name = "cpprest%s_%s_%s%s" % (
                toolset,
                version_tokens[0],
                version_tokens[1],
                debug_suffix,
            ...
            if not path.isfile(
                path.join(self.package_folder, "lib", "%s.lib" % versioned_name)
            ):
                versioned_name = "cpprest_%s_%s%s" % (
                    version_tokens[0],
                    version_tokens[1],
                    debug_suffix,
                )
