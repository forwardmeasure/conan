...
_cuda_version_installer_map = {
'10.2': 'cuda_10.2.89_440.33.01_linux.run'}
...
def get_cuda_download_location(version):
    ...
    return (installer, "http://developer.download.nvidia.com/compute/cuda/{}/Prod/local_installers/{}".format(version, installer))
...
def install_cuda(build_dir, install_script, install_dir):
    os.chdir(build_dir)
    install_cmd = "cd {} && bash {} --override --no-opengl-libs --silent --toolkit --toolkitpath={} --defaultroot={}".format(build_dir, os.path.join(os.getcwd(), install_script), install_dir, install_dir)
