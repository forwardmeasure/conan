...
class CppRestSDKConan(ConanFile):
    ...
    default_options = {"shared": True, "exclude_websockets": False, "fPIC": True, "exclude_compression": False}

    zlib_reference = "zlib/1.2.11@forwardmeasure/stable"
    boost_reference = "boost/1.72.0@forwardmeasure/stable"
    openssl_reference = "OpenSSL/1.1.1f@forwardmeasure/stable"
    websocketpp_reference = "websocketpp/develop@forwardmeasure/stable"
    ...
    def source(self):
        ...
        if self.settings.compiler == "clang" and str(self.settings.compiler.libcxx) in ["libstdc++", "libstdc++11"]:
            tools.replace_in_file(path.join(self._source_subfolder, "Release", "CMakeLists.txt"), "libc++", "libstdc++")
        if self.settings.os == "Android":
            tools.replace_in_file(
                path.join(self._source_subfolder, "Release", "cmake", "cpprest_find_boost.cmake"),
                "find_host_package",
                "find_package",
            )
            tools.replace_in_file(
            ...
            tools.replace_in_file(
                path.join(self._source_subfolder, "Release", "src", "http", "client", "http_client_asio.cpp"),
    ...
    def package_info(self):
        if self.settings.compiler == "Visual Studio":
            ...
            toolset = {"12": "120", "14": "140", "15": "141"}.get(str(self.settings.compiler.version))
