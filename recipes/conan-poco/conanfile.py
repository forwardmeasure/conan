...
class PocoConan(ConanFile):
    ...
    description = (
        "Modern, powerful open source C++ class libraries for building network- and internet-based "
    ...
    def requirements(self):
        if (
            ...
            or self.options.enable_netssl_win
            or self.options.enable_crypto
            or self.options.force_openssl
        ):
            self.requires.add("OpenSSL/1.1.1f@bottomline/stable", private=False)
    ...
    def package_info(self):
        ...
        suffix = (
            str(self.settings.compiler.runtime).lower()
