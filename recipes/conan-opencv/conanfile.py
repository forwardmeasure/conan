...
class OpenCVConan(ConanFile):
    ...
    def configure(self):
        ...
        if self.options.cuda and not self.options.contrib:
            raise ConanInvalidConfiguration("opencv:cuda requires opencv:contrib")
    ...
    def requirements(self):
        ...
        if self.options.jpeg:
            ...
            if self.options.jpegturbo:
                self.requires.add("libjpeg-turbo/2.0.2@bincrafters/stable")
            else:
        ...
        if self.options.openexr:
            self.requires.add("openexr/2.4.0@forwardmeasure/stable")
        if self.options.protobuf:
            ...
            self.requires.add("protobuf/3.10.1@forwardmeasure/stable")
        if self.options.eigen:
            self.requires.add("eigen/3.3.7@conan/stable")
        if self.options.gstreamer:
            self.requires.add("gstreamer/1.16.2@bincrafters/stable")
        ...
        if self.options.contrib:
            ...
            if self.options.harfbuzz:
                self.requires.add("harfbuzz/2.6.2@bincrafters/stable")
    ...
    def build(self):
        ...
        if str(self.settings.compiler) == "clang" and str(self.settings.compiler.version) == "3.9":
            tools.replace_in_file(
                os.path.join(self._source_subfolder, "modules", "imgproc", "CMakeLists.txt"),
        ...
        if patching:
            ...
            tools.patch(
                base_path="contrib", patch_file=os.path.join("patches", "0001-fix-find_package-for-glog-gflags.patch")
            )
