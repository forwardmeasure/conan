...
class LibtorchConan(ConanFile):
    ...
    def source(self):
        git_clone_command = "git clone https://github.com/pytorch/pytorch.git %s && cd %s && git checkout v%s && git submodule update --init --recursive" % (self._source_subfolder, self._source_subfolder, self.version)
    ...
    def _configure_cmake(self):
        env_build = dict()
        env_build["MAX_JOBS"] = "2"
        env_build["USE_NINJA"] = "OFF"
        with tools.environment_append(env_build):
            cmake = CMake(self, set_cmake_flags=True)
            cmake.verbose = True

            cmake.definitions["MAX_JOBS"] = "2"
            cmake.definitions["USE_NINJA"] = False
