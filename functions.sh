...
#   echo "Running GIT command": "LD_LIBRARY_PATH=${lib_path};DYLD_LIBRARY_PATH=${lib_path}" $gitRealPath "$@"
	LD_LIBRARY_PATH="${lib_path};DYLD_LIBRARY_PATH=${lib_path}" command git "$@"
}
...
function do_conan_install_old()
{
	...
	echo "Using extra build location '$extra_build_spec' for building"
	install_folder=${CONAN_USER_HOME}/CMakeModules/${extra_build_spec}
	conan_env_script=${install_folder}/activate_run.sh
	if [ -f "$conan_env_script" ]
	then
		echo "Sourcing file ${conan_env_script}"
		. ${conan_env_script}
	fi
	...
	cleanupPath PATH
	cleanupPath MANPATH
	cleanupPath LIBRARY_PATH
	cleanupPath DYLD_LIBRARY_PATH
	cleanupPath LD_LIBRARY_PATH
	cleanupPath CMAKE_PREFIX_PATH
